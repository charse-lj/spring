 ...
 * @see AdvisedSupport
 * AOP进化史
 * spring1硬编码(ProxyFactoryBean)-->spring2(AspectJ)-->spring3~5 自动代理
 * 用于创建代理的配置的父类，以确保所有代理创建者具有一致的属性
 *
 * ProxyFactoryBean、ProxyFactory、AspectJProxyFactory为这三个类提供配置属性
 */
public class ProxyConfig implements Serializable {
	...
	private boolean optimize = false;
	 /**
	  * 标记是否需要阻止通过该配置,将创建的代理对象转换为Advised类型，
	  ...
	  * 默认情况，我们可以这么完 Advised target = (Advised) context.getBean("opaqueTest"); 从而就可以对该代理持有的一些属性进行干预勒
	  * 若此值为true，就不能这么玩了
	  */
	boolean opaque = false;
	 /**
	  * 标记代理对象是否应该被aop框架通过AopContext以ThreadLocal的形式暴露出去。
	  * 当一个代理对象需要调用它【自己】的另外一个代理方法时，这个属性将非常有用。默认是false，以避免不必要的拦截。
	  */
