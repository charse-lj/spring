 ...
 * @since 3.0.4
 *
 * DefaultServletHttpRequestHandler和ResourceHttpRequestHandler,它和Spring MVC处理静态资源有非常大的关系。
 * 背景：如果将DispatcherServlet请求映射配置为"/"，则Spring MVC将捕获Web容器所有的请求，包括静态资源的请求，Spring MVC会将它们当成一个普通请求处理，因此找不到对应处理器将导致错误。
 * pring团队出了堪称经典的两种解决方案
 * 方法1.采用<mvc:default-servlet-handler /> @since 3.0.4,就是向MVC的容器内注入了一个DefaultServletHttpRequestHandler实例
