 ...
 * 对比BeanDefinition的源码我们可以发现，AbstractBeanDefinition对BeanDefinition的大部分方法做了实现（没有实现parentName相关方法）。
 * 同时定义了一系列的常量及默认字段。这
 * 是因为BeanDefinition接口过于顶层，如果我们依赖BeanDefinition这个接口直接去创建其实现类的话过于麻烦，所以通过AbstractBeanDefinition做了一个下沉，并给很多属性赋了默认值
 */
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * autowire-candidate属性设置为false，这样容器在查找自动装配对象时，将不考虑该bean，备注：并不影响本身注入其它的Bean
	...
	/**
	 * bd中,factoryBeanName、factoryMethodName何时有值?
	 * 第一种:
	 * @Configuration
	 * public class Config {
	 *    @Bean
	 ...
	 *    }
	 * }
	 * @Configuration、@Bean组合-->factoryBeanName=config，factoryMethodName=b
	 *
	 * 第二种:
	 ...
	 *
	 * xml --> factoryBeanName=config，factoryMethodName=b
	 *
	 * spring中的工厂bean分两种
	 ...
	 * 其实就是给resource赋值了，使用了BeanDefinitionResource
	 */
